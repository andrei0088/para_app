...
export default async function Raportedlanding({ searchParams }: PageProps) {
  const query = await searchParams;
  if (query?.delete) {
    await delete_landing_report(query.delete);
  }
  if (query?.notification) await erese_notification(query.notification, "l");

  const raports = await get_landing_report();
          ...
          >
            <div className="space-y-1">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm">
                ...
                <span>ID aterizare: </span> {r.landingId}{" "}
                <Link href={`/landing/${r.landingId}`}>Link</Link>
              </p>
            </div>
            {/* Form pentru ștergere notificare */}
            <form method="GET" action="/admin/raported/landing">
