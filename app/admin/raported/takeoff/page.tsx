...
export default async function Raportedtakeoff({ searchParams }: PageProps) {
  const query = await searchParams;
  if (query?.delete) {
    await delete_takeoff_report(query.delete);
  }
  if (query?.notification) await erese_notification(query.notification, "c");

  const raports = await get_takeoff_report();
          ...
          >
            <div className="space-y-1">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm">
                ...
                <span>ID decolare: </span> {r.takeoffId}{" "}
                <Link href={`/takeoff/${r.takeoffId}`}>Link</Link>
              </p>
            </div>
            {/* Form pentru ștergere notificare */}
            <form method="GET" action="/admin/raported/takeoff">
