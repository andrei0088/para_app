...
export default async function Raportedregion({ searchParams }: PageProps) {
  const query = await searchParams;
  if (query?.delete) {
    await delete_region_report(query.delete);
  }
  if (query?.notification) await erese_notification(query.notification, "c");

  const raports = await get_region_report();
          ...
          >
            <div className="space-y-1">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm">
                ...
                <span>ID regiune: </span> {r.regionId}{" "}
                <Link href={`/region/${r.regionId}`}>Link</Link>
              </p>
            </div>
            {/* Form pentru ștergere notificare */}
            <form method="GET" action="/admin/raported/region">
