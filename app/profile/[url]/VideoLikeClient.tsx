...
import { add_video_like, remove_video_like } from "./get_profil";
...
export default function VideoLikeClient({
  ...
  const handleAdd = async () => {
    if (loading) return;
    setUserLiked(true);
    setLike((prev) => prev + 1); // optimist update
    setLoading(true);
    try {
      ...
      {logd ? (
        userLiked ? (
          <div
            ...
            className="flex items-center gap-1 py-1 px-2 border-2 rounded-sm cursor-pointer
                     ...
                     animate-[pulse_2.5s_ease-in-out_infinite] hover:animate-none text-xs md:text-sm"
          >
