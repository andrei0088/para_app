...
// ============================================
// Get Takeoff Details
export async function get_takoff_details({
  id,
}: {
  id: number;
...
// ============================================
// Get Landing Details
export async function get_landing_details({
  id,
}: {
  id: number;
}): Promise<Details | null> {
...
export async function get_regions_by_season({
  id,
}: {
  id: number;
}): Promise<Region[]> {
  ...
  return regions.filter(
    (r) => r.bestSeason?.some((m) => months.includes(m)) ?? false
  );
}

export async function get_regions_by_month({
  id,
}: {
  id: number;
}): Promise<Region[]> {
